...
export function commandsToGraph(commands: Command[]): WorkflowGraph {
  ...
  for (const { keyword, label } of commands) {
    if (ended) {
      throw new WorkflowGraphError(`Heard "${keyword}" after "end" — workflow is already closed`);
    }

    switch (keyword) {
      case 'start': {
        if (startId) throw new WorkflowGraphError('Heard a second "start" — a workflow can only have one');
      ...
      case 'subagent': {
        if (!lastAgentId) throw new WorkflowGraphError('Heard "subagent" before any "agent"');
      ...
      case 'end': {
        if (!startId) throw new WorkflowGraphError('Heard "end" before "start"');
        mainStep += 1;
        const id = nextId('end');
